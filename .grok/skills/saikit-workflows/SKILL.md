---
name: saikit-workflows
description: |
  Configure SummonAI Kit workflow integrations, detects tools, asks how the team uses them, installs ops skills + writes a custom dev-cycle playbook. Idempotent. Invoked via /saikit-workflows.
when-to-use: saikit workflows, configure summonaikit, install skills via workflow, dev-cycle playbook
user_invocable: true
allowed-tools: run_terminal_command, ask_user_question, read_file, write
---
...
| `kind` | What to do |
|---|---|
| `ask` | Call **ask_user_question** with `instruction.question` (it has `question`, `multiSelect`, `options`). Store single-select answers as one string value. Store multi-select answers as a JSON array of selected option values. Then loop. |
...
| `bash` | Run `instruction.command` via **run_terminal_command**. Surface its stdout to the user (one line is fine). Then loop. |
...
- `.grok/skills/` exists → `grok` is a target (Grok native)
- Default to `claude` + `cursor` + `grok` if present
...
**Grok note**: This is the Grok-native version. It uses `run_terminal_command`, `ask_user_question`, `write`, and `read_file`. The binary will install into `.grok/skills/` when `grok` is in the targets list.
