...
return [
    ...
    */
    'datasets' => [
        'stellar' => [
            ...
            'api_key' => env('BRIDGE_STELLAR_KEY', env('BRIDGE_API_KEY')),
            'api_secret' => env('BRIDGE_API_SECRET'),
            'replication_top' => min(2000, max(1, (int) env('BRIDGE_SYNC_REPLICATION_TOP', 2000))),
            'incremental_top' => min(200, max(1, (int) env('BRIDGE_SYNC_INCREMENTAL_TOP', 200))),
            'persist_chunk_size' => min(250, max(25, (int) env('BRIDGE_SYNC_PERSIST_JOB_CHUNK', 50))),
            'upsert_chunk_size' => min(500, max(25, (int) env('BRIDGE_SYNC_UPSERT_CHUNK', 250))),
            'fetch_queue' => env('BRIDGE_SYNC_FETCH_QUEUE', 'bridge-sync-fetch'),
            'persist_queue' => env('BRIDGE_SYNC_PERSIST_QUEUE', 'bridge-sync-persist'),
            'rate_limit_rps' => min(10, max(1, (int) env('BRIDGE_SYNC_MAX_REQUESTS_PER_SECOND', 2))),
        ...
        'beaches' => [
            ...
            'api_key' => env('SPARK_BEACHES_KEY', env('SPARK_ACCESS_TOKEN')),
            'replication_top' => min(1000, max(1, (int) env('SPARK_SYNC_REPLICATION_TOP', 1000))),
            'incremental_top' => min(1000, max(1, (int) env('SPARK_SYNC_INCREMENTAL_TOP', 1000))),
            'expand' => (string) env('SPARK_SYNC_EXPAND', 'Media,Unit,Room,OpenHouse'),
            'persist_chunk_size' => min(250, max(25, (int) env('SPARK_SYNC_PERSIST_JOB_CHUNK', 25))),
            'upsert_chunk_size' => min(500, max(25, (int) env('SPARK_SYNC_UPSERT_CHUNK', 250))),
            'fetch_queue' => env('SPARK_SYNC_FETCH_QUEUE', 'spark-sync-fetch'),
            'persist_queue' => env('SPARK_SYNC_PERSIST_QUEUE', 'spark-sync-persist'),
            'rate_limit_rps' => min(10, max(1, (int) env('SPARK_SYNC_MAX_REQUESTS_PER_SECOND', 2))),
    ...
    */
    ...
    'replica_page_retention_hours' => max(1, (int) env('MLS_REPLICA_PAGE_RETENTION_HOURS', 24)),
    'replica_page_failed_retention_days' => max(1, (int) env('MLS_REPLICA_PAGE_FAILED_RETENTION_DAYS', 7)),
