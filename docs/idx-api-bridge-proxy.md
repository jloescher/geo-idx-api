...
| **Replica sync** | **`mls:replication-kickoff`** (every minute) starts catch-up when the mirror is **>15 minutes** behind MLS “now”; steady state schedules incremental fetches with a **15-minute** idle delay. `BridgeSyncFetchPageJob` runs on **`bridge-sync-fetch`** (max **2 Bridge GETs/sec**). Fetched pages are stored in **`replica_pages`** (`provider=bridge`, multi-part gzip JSON) then persisted on **`bridge-sync-persist`**. Replication uses **`MLS_SYNC_EXPAND`** (default `Media,Unit,Room,OpenHouse`) on Bridge and Spark. **Incremental** sync filters on the dataset-appropriate OData field (`BridgeModificationTimestamp` for Stellar, `ModificationTimestamp` for Beaches) against **`last_modification_timestamp`**. **Closed** is not bulk-replicated—use live Bridge for on-demand Closed queries. Mirror-backed search merges `raw_data`, expanded JSONB columns, and flat-merged `custom_fields` into one RESO Property object. Completed staging rows are purged on finalize and via **`mls:purge-replica-pages`** (alias `bridge:purge-replica-pages`, daily). |
