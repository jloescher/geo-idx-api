...
| **Replica sync** | **`mls:replication-kickoff`** (every minute) starts catch-up when the mirror is **>15 minutes** behind MLS “now”; steady state schedules incremental fetches with a **15-minute** idle delay. `BridgeSyncFetchPageJob` runs on **`bridge-sync-fetch`** (max **2 Bridge GETs/sec**). Fetched pages are stored in **`replica_pages`** (`provider=bridge`, multi-part gzip JSON) then persisted on **`bridge-sync-persist`**. Replication seed uses OData **`$filter`** for **Active + Pending** only and always includes **`Media`** in `$select`. After catch-up, **incremental** sync uses `ModificationTimestamp gt datetime'{last_bridge_modification_timestamp}'` so pages are not re-requested. **Closed** is not bulk-replicated—use live Bridge for on-demand Closed queries. Completed staging rows are purged on finalize and via **`mls:purge-replica-pages`** (alias `bridge:purge-replica-pages`, daily). |
