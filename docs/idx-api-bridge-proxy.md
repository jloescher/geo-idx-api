...
| Hybrid map performance | `POST /api/v1/search`: Active/Pending from PostGIS replica; Closed from live Bridge; mixed status filters merge both (see **Hybrid search** under Caching & jobs). |
